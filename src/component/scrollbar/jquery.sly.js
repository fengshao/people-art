...
// Plugin names
	...
	function Plugin(frame, o) {
		...
		// Scrollbar variables
			$sb = $(o.scrollBar).eq(0),
			$sb1 = $(o.scrollBar).eq(1),
			$handle = $sb.length ? $sb.children().eq(0) : 0,
			$handle1 = $sb1.length ? $sb1.children().eq(0) : 0,
			sbSize = 0,
			handleSize = 0,
			hPos = {
				cur: 0,
		...
		this.destroy = function () {
			...
			// Unbind all events
			$frame.add(document).add($slidee).add($items).add($scrollSource).add($handle).add($handle1)
				.add($prevButton).add($nextButton).add($prevPageButton).add($nextPageButton)
				.unbind('.' + namespace);

			// Reset some styles
			$slidee.add($handle).css(o.horizontal ? {left: 0} : {top: 0});
			$slidee.add($handle1).css(o.horizontal ? {left: 0} : {top: 0});
		...
		(function () {
			...
			$sb1.css('position') === 'static' && $sb1.css({position: 'relative'});
			$slidee.add($handle).css(o.horizontal ? {position: 'absolute', left: 0} : {position: 'absolute', top: 0});
			$slidee.add($handle1).css(o.horizontal ? {position: 'absolute', left: 0} : {position: 'absolute', top: 0});
			...
			// Navigation buttons
			o.prev && $prevButton.bind('click.' + namespace, function (e) {
				stopDefault(e);
				self.prev();
			});
			o.next && $nextButton.bind('click.' + namespace, function (e) {
				stopDefault(e);
				self.next();
			});
			o.prevPage && $prevPageButton.bind('click.' + namespace, function (e) {
				stopDefault(e);
				self.prevPage();
			});
			o.nextPage && $nextPageButton.bind('click.' + namespace, function (e) {
				stopDefault(e);
				self.nextPage();
			});
